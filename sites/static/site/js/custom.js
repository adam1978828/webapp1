...
var $block = $('<div/>', {
    'class': 'top-scroll'
}).append('<a href="#"/>').hide().appendTo('body').click(function() {
    $('body,html').animate({
        scrollTop: 0
    }, 800);
    return false;
...
var nav = $('header').height() + 'px';
$('.wrapper').attr('style', "padding-top:" + nav);
//handlers    
...
// Close all window and open one
var changeWindow = function(win) {
    win = typeof win !== 'undefined' ? win : false;

    $('.alertmodal').removeClass('open').addClass('close');
    $('.signo').removeClass('open').addClass('close');
    $('.signupo').removeClass('open').addClass('close');
    $('.checko').removeClass('open').addClass('close');
    $('.forgoto').removeClass('open').addClass('close');
    $('.kioskselect').removeClass('open').addClass('close');
    $('.kioskchoose').removeClass('open').addClass('close');
    $('#reservationSuccessOverlay').removeClass('open').addClass('close');
    $('.card').removeClass('open').addClass('close');
    $('.kioskchoosecart').removeClass('open').addClass('close');
    $('.kioskselectcart').removeClass('open').addClass('close');
    $('#couponCodeModal').removeClass('open').addClass('close');
    $("#normal-forgot").show();
    $("#success-forgot").hide();
    $(".btn--wider").removeAttr('disabled');
    $("#normal-reg").show();
    $("#success-reg").hide();
    setTimeout(function() {
        $('.signo').removeClass('close');
        $('.signupo').removeClass('close');
        $('.alertmodal').removeClass('close');
        $('.checko').removeClass('close');
        $('.card').removeClass('close');
        $('.kioskchoosecart').removeClass('close');
        $('.kioskselectcart').removeClass('close');
        $('.forgoto').removeClass('close');
        $('.kioskselect').removeClass('close');
        $('#reservationSuccessOverlay').removeClass('close');
        $('#couponCodeModal').removeClass('close');
    }, 500);

    if (win){
        $('.' + win).removeClass('close').addClass('open');
    }
};
//changeWindow();
//4. Login window pop up
//Login pop up
$('.login-window, .sign').click(function(e) {
    e.preventDefault();
...
$('.check').click(function(e) {
    e.preventDefault();
    $('.checko').removeClass('close').addClass('open');
    $('.signo').removeClass('open').addClass('close');
    $('.forgoto').removeClass('open').addClass('close');
});
$('.close-select').click(function(e) {
    $('.kioskselectcart').removeClass('open').addClass('close');
});

$('.forgot').click(function(e) {
    e.preventDefault();
    $('.forgoto').removeClass('close').addClass('open');
    $('.checko').removeClass('open').addClass('close');
    $('.signo').removeClass('open').addClass('close');
    setTimeout(function() {
        $('.checko').removeClass('close');
...
$('.choose').click(function(e) {
    e.preventDefault();
    $('.kioskchoose').removeClass('close').addClass('open');
});
...
var heightHtml = $('html').height() - $('.footer-wrapper').height();
if (heightHtml > 0) {
    $('#fixfooter').attr('style', 'min-height:' + heightHtml + 'px;');
} else {
    $('#fixfooter').attr('style', 'min-height:auto;');
}
$(window).resize(function() {
    heightHtml = $('html').height() - $('.footer-wrapper').height();
    if (heightHtml > 0) {
        $('#fixfooter').attr('style', 'min-height:' + heightHtml + 'px;');
    } else {
        $('#fixfooter').attr('style', 'min-height:auto;');
...
function init_Elements() {
    ...
    function init_progressBar(duration) {
        $('.progress').each(function() {
            ...
            var result = value + '%';
            if (duration) {
                $(this).find('.progress__current').animate({
                    width: value + '%'
                }, duration);
            } else {
                $(this).find('.progress__current').css({
                    'width': value + '%'
                });
            }
...
function init_Home() {
    ...
    api.bind("revolution.slide.onchange", function(e, data) {
        ...
        var nextSlide = slides.eq(currentSlide).attr('data-slide2');
        var prevSlide = slides.eq(currentSlide - 2).attr('data-slide2');
...
function init_Contact() {
    ...
    function setMarkers(map, locations) {
        if (kiosks.length > 0) {
            ...
            for (var i = 0; i < locations.length; i++) {
                var kiosk = locations[i];
                var myLatLng = new google.maps.LatLng(kiosk[1], kiosk[2]);
                ...
                google.maps.event.addListener(marker, "click", function(e) {
                    $('.kioskpattern').find('#kiosklink a').attr('href', this.link);
                    $('.kioskpattern').find('#kioskaddress').text(this.address);
                    $('.kioskpattern').find('#kioskcity').text(this.city);
                    $('.kioskpattern').find('#kioskstate').text(this.state);
                    $('.kioskpattern').find('#kioskpostalcode').text(this.postalcode);
                    $('.kioskpattern').find('#kioskname').text(this.title);
                    var boxTextA = $('.kioskpattern').html();
                    var infoboxOptionsA = {
                        content: boxTextA,
                        ...
                        closeBoxURL: "/static/site/images/components/close.svg",
                        isHidden: false,
                        pane: "floatPane",
    ...
    if (kiosks.length > 0) {
        var mapOptions = {
            scaleControl: true,
            center: new google.maps.LatLng(kiosks[0][1], kiosks[0][2]),
            zoom: 8,
...
function init_KioskDiscsMovies() {
    "use strict";
    ...
    $(".select__sort").selectbox({
        onChange: function(val, inst) {
            ...
            var option = $(inst.input[0]).find('[value="' + val + '"]');
            if (option.attr('href')) {
...
function init_KioskDiscs() {
    ...
    function setMarkers(map, locations) {
        ...
        if (window.hasOwnProperty('kiosks')) {
            for (var i = 0; i < locations.length; i++) {
                var kiosk = locations[i];
                var myLatLng = new google.maps.LatLng(kiosk[1], kiosk[2]);
                var marker = new google.maps.Marker({
    ...
    if (window.hasOwnProperty('kiosks')) {
        ...
        var map = new google.maps.Map(document.getElementById('location-map-kiosk'), mapOptions);
        setMarkers(map, kiosks);
        var map_style = [{
            stylers: [{
                saturation: -100
...
function init_KiosksMap() {
    ...
    function setMarkers(map, locations) {
        ...
        for (var i = 0; i < locations.length; i++) {
            ...
            var marker = new google.maps.Marker({
                ...
                //icon: image,
                shape: shape,
                title: decodeURIComponent(escape(kiosk[0])),
                address: decodeURIComponent(escape(kiosk[4])),
                city:decodeURIComponent(escape(kiosk[5])),
                state:decodeURIComponent(escape(kiosk[6])),
                postalcode:decodeURIComponent(escape(kiosk[7])),
                link: kiosk[3]
                //zIndex: kiosk[3]
            });
            google.maps.event.addListener(marker, "click", function(e) {
    ...
    function getFirstKioskCoordinates(kiosks) {
        for (var i in kiosks) {
            if (kiosks[i][1] != '' && kiosks[i][2] != '')
                return [kiosks[i][1], kiosks[i][2]]
        }
        return undefined
    }

    var centerKiosk = getFirstKioskCoordinates(kiosks);
    if (centerKiosk === undefined)
        centerKiosk = [-34.397, 150.644]
    var mapOptions = {
        scaleControl: true,
        center: new google.maps.LatLng(centerKiosk[0], centerKiosk[1]),
        zoom: 8,
        navigationControl: false,
        streetViewControl: false,
        mapTypeControl: true,
...
function init_Cart() {
    ...
    $('#cardNumber').inputmask("9{12,20}");
    $('#cardExpiry').inputmask("9{2,2}9{2,2}");
    $(".select__sort").selectbox({
        onChange: function(val, inst) {
            // console.log(val, inst)
            $(inst.input[0]).children().each(function(item) {
    ...
    $('a.choose').removeClass('choose').addClass('chooseCart').unbind('click');
    $('.chooseCart').live('click', function(e) {
        e.preventDefault();
        $('.kioskchoosecart').removeClass('close').addClass('open');
    });
    $('.namekiosk a, .address .btn-sm').click(function(e) {
        e.preventDefault();
        var self = $(this);
        var info = "";
        var activeb = "";
        var nameKiosk = "";
        var href = $('#choose-kiosk-form').attr('action');
        var kioskId = self.attr('data-id');
        var redirectUrl = self.attr('data-href');
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data.type == "error") {} else if (data.type == "success") {
                    ...
                    $('.kioskchoosecart').removeClass('open').addClass('close');
                    info = $('#normal-kioskchoosecart .address .btn--info:first').text();
                    activeb = $('#normal-kioskchoosecart .address .btn--danger').text();
                    nameKiosk = self.parent().parent().find('.namekiosk a').text();
                    $('#normal-kioskchoosecart .address .btn-sm').removeClass('btn--danger').addClass('btn--info').text(info);
                    self.removeClass('btn--info').addClass('btn--danger').text(activeb);
                    $('.chooseCart').text(nameKiosk);
                }
            },
            error: function() {
                alert("FAIL!");
...
$('#search-form .sbOptions a').click(function() {
...
$('.no-kiosk').click(function(e) {
    e.preventDefault();
    $('.kioskselectcart').removeClass('close').addClass('open');
});

$('.namekiosk a, .address .btn-choose').click(function(e) {
    e.preventDefault();
    var self = $(this);
    var href = $('#choose-kiosk-form').attr('action');
    var kioskId = self.attr('data-id');
