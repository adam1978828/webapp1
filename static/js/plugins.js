...
(!window.console || !console.log) && function() {
    for (var c = function() {}, a = "assert clear count debug dir dirxml error exception group groupCollapsed groupEnd info log markTimeline profile profileEnd markTimeline table time timeEnd timeStamp trace warn".split(" "), b = a.length, d = window.console = {}; b--;) d[a[b]] = c
}();
...
(function($, undefined) {
    $.fn.equalHeight = function() {
        var $this = $(this);
        $this.height(Math.max.apply(Math, $this.map(function() {
            return $(this).height();
        }).get()));
        return $this;
    }
    ...
    $.fn.equalWidth = function() {
        var $this = $(this);
        $this.width(Math.max.apply(Math, $this.map(function() {
            return $(this).width();
        }).get()));
        return $this;
    }
...
(function($, window, document) {
    ...
    $.fn[PLUGIN_NAME] = function(options) {
        ...
        return $(this).each(function() {
            ...
            $table.dataTable($.extend(true, {
                ...
                iDisplayLength: 25,
            }, opts.dataTable)).parents('.dataTables_wrapper').find('.dataTables_length select').data('width', 'auto');

            var $dataTable = $table.parents('.dataTables_wrapper'),
                $filterBar = $dataTable.find('.filters');

            ['with-prev-next', 'full'].forEach(function(cls) {
                $table.hasClass(cls) && $dataTable.addClass(cls);
            });
            $dataTable.find('.dataTable.full').removeClass('full');

            // ! Toolbar
            $dataTable.prev('.tabletools').prependTo($dataTable);
            ...
            if (opts.tableTools.display || $dataTable.find('.tabletools').length) {
                if (opts.tableTools.display && !(Modernizr.touch && !opts.tableTools.enabledOnTouchDevices)) {
                    ...
                    $dataTable.data('tableTools', tableTools);
                }
            ...
            if (opts.filterBar != 'none') {
                ...
                // Move to top
                $dataTable.find('.tabletools').insertAfter($dataTable.find('.filters'));
            }
    ...
    $.fn[PLUGIN_NAME].defaults = {
        ...
        tableTools: {
            ...
            enabledOnTouchDevices: navigator.mimeTypes && navigator.mimeTypes.length ? Array.prototype.slice.call(navigator.mimeTypes).some(function(a) {
                return "application/x-shockwave-flash" == a.type
            }) : /MSIE/.test(navigator.userAgent) ? eval("try{new ActiveXObject('ShockwaveFlash.ShockwaveFlash')&&!0}catch(e){!1};") : !1 // Flash detection
...
(function($, window, document) {
    ...
    $.fn[PLUGIN_NAME] = function(options) {
        ...
        return $(this).each(function() {
            ...
            bg.onload = function() {
                maxBgPositionX = bg.width - $indicator.width()
            }; // FIX for webkit
...
(function($, _, window, document, undefined) {
    ...
    var PLUGIN_NAME = 'search',
        ns = '.mango_' + PLUGIN_NAME;
    ...
    $.fn[PLUGIN_NAME] = $.extend(function(method, options) {
        ...
        $(this).each(function() {
            ...
            // ! Create instance
            if (inst = instances.filter(function(o) {
                return o.el[0] == $el[0];
            })[0]) {
                inst = inst.inst
            } else {
            ...
            method != 'init' && !inst.get('initialized') && $.error('$.fn.' + PLUGIN_NAME + ' was not initialzed. Please call $.fn.' + PLUGIN_NAME + '(options) first.');

            if (method == 'init' && inst.get('initialized')) {
                return;
            }
    ...
    function self() {
        ...
        $.extend(me, {
            ...
            search: function() {
                ...
                me.xhr /* Store the ajax request */ = $.post(me.options.source, {
                    term: term,
                    max: me.options.maxResults
                }, function(results) {
                    var htmlArray = [];
...
(function($, window, undefined) {
    ...
    $.fn.infobar = function(opts) {
        ...
        return $(this).each(function() {
            ...
            // ! Collect settings
            var $bar = $(this),
                data = $.extend({}, $.fn.infobar.defaults, opts, $bar.data(), {
                    colorize: false
                });
            ...
            var $progress = $bar.append($('<div>', {
                'class': 'small'
            }).progressbar(data)).children('div.ui-progressbar');
            ...
            $progress
                .bind('progressbarchange', function(e) {
                    ...
                    // Get new data
                    data = $bar.data($progress.data('uiProgressbar').options).data();
    ...
    function self() {
        ...
        $.extend(self, {
            ...
            init: function(opts) {
                ...
                $(window).trigger('lockscreenReady');
    ...
    function self() {
        ...
        $.extend(me, {
            ...
            init: function(opts) {
                ...
                var $container = $('<div>', {
                    'class': 'chart',
                    height: me.options.height || $this.parent().height()
                }).insertBefore($this);
                ...
                // ! Set up legend
                me.options.legend && (me.$legend_box = $('<div>', {
                    'class': 'legend'
                }).insertBefore($this));
                ...
                if (me.options.height) {
                    ...
                    $this.height(
                        $this.parent().height() - paddingTopBottom - (me.options.legend ? me.$legend_box.height() : 0) - (me.options.showTable ? $table.height() : 0)
                    );
                }
                    ...
                    $.extend(true, opts.flot, {
                        series: {
                            ...
                            lines: {
                                ...
                                fillColor: (opts.fill == false) ? null : {
                                    colors: [{
                                        opacity: 0
                                    }, {
                                        opacity: 0.15
...
(function($, _, window, document, undefined) {
    ...
    $.fn[PLUGIN_NAME] = $.extend(function(method, options) {
        ...
        $(this).each(function() {
            ...
            // ! Create instance
            if (inst = _.filter(instances, function(o) {
                return o.el[0] == $el[0];
            })[0]) {
                inst = inst.inst
            } else {
    ...
    function self() {
        ...
        $.extend(me, {
            ...
            destroy: function() {
                me.$element.off(ns);
                me.$element.find(me.options.selector).children().remove();
            },

            update: function() {
                me.destroy();
                me.init.apply(me.$element, me.options);
            },
            ...
            minichart: function($stat) {
                ...
                $stat.find('.minichart').sparkline(opts.values, {
                    type: 'bar',
                    barColor: color,
                    negBarColor: color,
            ...
            level: function($stat) {
                ...
                // Build html
                // - Build gauge
                var html = '<div class="gauge-container">' + '<div class="gauge' + (opts.value == opts.max ? ' full' : '') + '" style="height:' + (opts.value / opts.max * 100) + '%"></div>' + '</div>';
                $stat.insert(html);

                // - Build info
                var htmlInner = '<div class=max>' + me.format(opts.max, opts.format.toString()) + '</div>';
                htmlInner += '<div class=info>' + '<div class=value>' + me.format(opts.value, opts.format.toString()) + '</div>' + '<div class=description>' + opts.description + '</div>' + '</div>';
            ...
            pillar: function($stat) {
                ...
                $.each(opts.data, function() {
                    ...
                    htmlInner += '<div class="inner ' + (this.color || 'green') + '" style="height:' + height + 'px">' + '<div class=level></div>' + '<div class=value>' + me.format(this.val, opts.format.toString()) + '</div>' + '<div class=title>' + this.title + '</div>' + '</div>';
                });
                ...
                var valuesRightMax = Math.max.apply(Math, $values.map(function() {
                    var $el = $(this);
                    return $el.position().left + $el.width()
                }).get()) + parseInt($values.css('padding-right'));
                var $titles = $pillar.find('.title').css('left', valuesRightMax + 'px')

                var titlesMaxWidth = Math.max.apply(Math, $titles.map(function() {
                    return $(this).width();
                }).get());
            ...
            circular: function($stat) {
                ...
                if (opts.list) {
                    ...
                    opts.value = opts.list.map(function(o) {
                        return _.isNumber(o.percent) ? o.percent : o.val
                    }).reduce(function(a, b) {
                        return a + b
                    });
                }
            ...
            uptime: function($stat) {
                ...
                $.each(opts.servers, function() {
                    ...
                    stateHtml += '<div class="status ' + (this.online ? 'online' : 'offline') + '">' + (this.online ? opts.lang.online : opts.lang.offline) + '</div>';
            ...
            hlist: function($stat) {
                ...
                $.each(opts.list, function() {
                    ...
                    if (this.data_type == 'string'){
                        entry += '<div class=value>' + this.val + '</div>';
                    } else{
                        entry += '<div class=value>' + me.format(this.val, this.format || '') + '</div>';
                    }
        ...
        $.extend(me, {
            ...
            prepareHtml: function() {
                ...
                _.each(me.buttons, function($el, type) {
                    $el.click(handler(me[type.replace('$', '')]));
                });

                // ! Set up doubleclick handlers
                me.$left.dblclick(handler(me.add));
                me.$right.dblclick(handler(me.del));
            ...
            reflectBack: function() {
                ...
                me.$element.change(); // Fire change event
