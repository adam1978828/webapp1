...
class FirstData(object):
    ...
    def __init__(self, key, secret, gateway_id, password, test=False):
    ...
    def process(self, httpclient=None, callback=None, verbose=None,
                retry_on_bmc=1, **payment_arguments):
        ...
        if httpclient is not None:
            ...
            httpclient.fetch(("https://" + (self.GATEWAY_TEST if self._test else self.GATEWAY_LIVE) + "/transaction/v19"),
            ...
            # Synchronous
            r = requests.post(("https://" + (self.GATEWAY_TEST if self._test else self.GATEWAY_LIVE) + "/transaction/v19"),
                              timeout=20,
                              verify=not self._test,
                              data=transaction_body,
                              headers=headers)
            return self.process_repsonse(r, payment_arguments)

    def process_repsonse(self, response, payment_arguments):
            ...
            try:
                ...
                try:
                    ...
                    error = {
                        ...
                        "bank_message": response,
                        "amount": self._arguments.get('amount', 0),
                        ...
                        "card_cost": None,
                        "exact_message": response,
                        "logon_message": None,
                        "secure_auth_result": None,
                        "payer_id": None,
                        "transaction_type": self._arguments.get(
                            'transaction_type'),
                        "cc_verification_str2": None,
                        "ecommerce_flag": None,
                        "reference_no": self._arguments.get('reference_no'),
                        "cavv": None, "previous_balance": None,
                        "error_description": None, "tax2_number": None,
                        "exact_resp_code": None, "secure_auth_required": None,
                        "amount_requested": None, "client_email": None,
                        "cc_verification_str1": None, "language": None
                    }
                    if self._callback:
